...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="text-center mb-8">
        <h1 class="text-3xl font-black text-theme-dark">Create New Category</h1>
        <p class="text-gray-500 mt-1">Add a new group for your products.</p>
    </div>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6 relative z-10">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" 
                       class="w-full border-theme-soft rounded-xl focus:ring-theme-main focus:border-theme-main py-3 px-4 bg-gray-50 focus:bg-white transition-colors placeholder-gray-400"
                       placeholder="e.g. Summer Collection">
                @error('name')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-bold text-theme-dark mb-2">Slug (Optional)</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400 text-sm font-mono">/</span>
                    <input type="text" name="slug" value="{{ old('slug') }}" 
                           class="w-full pl-6 border-theme-soft rounded-xl focus:ring-theme-main focus:border-theme-main py-3 px-4 bg-gray-50 focus:bg-white transition-colors placeholder-gray-400"
